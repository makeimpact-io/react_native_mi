...
import {
  ...
  getTop100Companies,
} from '../helpers/investingExploreHelper';
import { InvestsNavigationParamList } from '../../../navigation/App/SubNavigations/InvestNavigation';

type Props = ReturnType<typeof mapStateToProps> &
  MaterialTopTabScreenProps<InvestsNavigationParamList, 'ExploreStocks'>;

const InvestingExploreScreen = (props: Props) => {
      ...
      <View style={styles.horizontalScrollItem} key={company.id}>
        <ImpactChampion
          ...
          onClick={() =>
            props.navigation
              .getParent()
              ?.navigate('CompanyDetailsStocks', { company: company })
          }
        />
      </View>
    );
      ...
      <View style={styles.sectorLinkContainer} key={sector.id}>
        <DefaultButton
          ...
          action={() =>
            props.navigation
              .getParent()
              ?.navigate('InvestingSectorCompanies', { sector: sector })
          }
        />
      </View>
    );
        ...
        <ScrollView
            ...
            <View style={styles.sectorHeadlineContainer}>
              ...
              <TouchableWithoutFeedback
                onPress={() =>
                  props.navigation
                    .getParent()
                    ?.navigate('InvestingCommitmentCompanies', {
          ...
          <View style={styles.viewAllButtonContainer}>
            <DefaultButton
              ...
              allCaps={true}
              action={() =>
                props.navigation.getParent()?.navigate('InvestingCommitments')
              }
            />
          </View>
          <View style={styles.sectorHeadlineContainer}>
            ...
            <TouchableWithoutFeedback
              onPress={() =>
                props.navigation.getParent()?.navigate('InvestingSectors')
              }>
              <Text style={styles.viewAllButton}>View All</Text>
            </TouchableWithoutFeedback>
          </View>
...
const InvestingExploreScreenConnected = connect(
  mapStateToProps,
  mapDispatchToProps,
)(InvestingExploreScreen);
...
export { InvestingExploreScreenConnected as InvestingExploreScreen };
